...
typedef struct _STRU_MEETING_EXIT_NOTIFY {
    ...
    int userId;
    char userName[_DEF_MAX_LENGTH];
    int isMeetingEnd; // 1=主持人退出会议结束, 0=仅成员离开
...
typedef struct _STRU_FRIEND_STATUS_NOTIFY {
    packType type;
    int userId;     //状态变更的好友ID
    int status;     //_DEF_STATUS_ONLINE 或 _DEF_STATUS_OFFLINE
...
typedef struct _STRU_FRIEND_REQUEST_ACTION_RQ {
    ...
    int userId;
    int requestId;
    int action; // _DEF_REQUEST_ACCEPT 或 _DEF_REQUEST_REFUSE_ACTION
    char friendName[_DEF_MAX_LENGTH];
